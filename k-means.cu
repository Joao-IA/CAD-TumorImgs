...
int main() {
    ...
    for (int ik = 0; ik < num_ks; ++ik) {
        ...
        for (int j = 0; j < ks[ik]; ++j) {
            int idx = dist_pt(rng);
            h_centroids[(base + j) * dim] = h_data[idx * dim];
        }
    }
    ...
    while (iter < MAX_ITER && !converged) {
        ...
        assign_clusters_multiK<<<grid, block>>>(
            d_data, n, dim,
            d_centroids, d_ks, d_offsets,
            num_ks, d_labels
        );
        cudaDeviceSynchronize();

        update_centroids_multiK<<<grid, block>>>(
            d_data, n, dim,
            d_labels,
        ...
        for (int ik = 0; ik < num_ks; ++ik) {
            ...
            for (int j = 0; j < ks[ik]; ++j) {
                int idx = base + j;
                if (h_counts[idx] > 0) {
                    float newc = h_new_centroids[idx * dim] / h_counts[idx];
                    if (std::fabs(newc - h_centroids[idx * dim]) > EPS) converged = false;
                    h_centroids[idx * dim] = newc;
                }
            }
        }
        cudaMemcpy(d_centroids, h_centroids.data(), K_total * dim * sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (int ik = 0; ik < num_ks; ++ik) {
        ...
        int base = offsets[ik];
        for (int j = 0; j < ks[ik]; ++j) {
            std::cout << h_centroids[(base + j) * dim] << " ";
        }
        std::cout << "\n";
    }
